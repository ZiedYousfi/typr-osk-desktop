...
private:
  ...
  void handleEvent(const KBDLLHOOKSTRUCT *kbd, bool pressed) {
    ...
    if (output_debug_enabled()) {
      ...
      fprintf(stderr,
              "[typr-backend] OutputListener (Windows) %s: vk=%u key=%s cp=%u "
              "mods=%u\n",
  ...
  void threadMain() {
    ...
    if (hook == nullptr) {
      ...
      ready.store(false);
      if (output_debug_enabled()) {
        fprintf(stderr, "[typr-backend] OutputListener (Windows): "
    ...
    ready.store(true);
    if (output_debug_enabled()) {
      fprintf(stderr, "[typr-backend] OutputListener (Windows): low-level "
...
std::atomic<OutputListener::Impl *> OutputListener::Impl::s_instance{nullptr};
